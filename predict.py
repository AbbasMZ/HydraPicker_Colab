...
def start():
    ...
    if prediction_head == 'all':
        aux.aux.Tparticle[0] = heads_dict['gen'] * torch.eye(1, dtype=torch.int8)    
    ...
    if prediction_head != 'all':
        ...
        aux.aux.heads_eval_mode(learn.model[1])
        start_time = time.time()
        aux.aux.Tparticle[0] = heads_dict[target_datasets[0]] * torch.eye(1, dtype=torch.int8)

        with torch.no_grad():
            for val_counter in range(len(val_idxs)):
                learn.set_data(md_target_datasets_sep_tst0)
                x0, y0 = next(iter0)
                aux.aux.Tparticle[0] = int(y0[2].cpu().numpy()[0]) * torch.eye(1, dtype=torch.int8)
                x0, y0 = add_border(x0, y0)
                x0 = V(x0)
                pred0 = learn.model(x0)
