...
def start():
    ...
    parser.add_argument('-di', '--dest_image', type=str, default='trainingdata8_b', dest='dest_image')
    ...
    for c1 in range(range_start, range_end):
        ...
        if mode == 'image':
            ...
            for c2 in range(len(centroids)):
                if c2 % 2.0 > 0:  # height
                    centroids2 = ' '.join([centroids2, str(int(np.floor((int(centroids[c2]) + border_size) * height_ratio)))])
                else:  # width
                    centroids2 = ' '.join([centroids2, str(int(np.floor((int(centroids[c2]) + border_size) * width_ratio)))])
