...
# def near_center_crop(im, min_sz=None):
#     """ Returns a near center crop of an image"""
#     r,c,*_ = im.shape
#     if min_sz is None: min_sz = min(r,c)
#     distance_r = np.random.randint(-10, 10)
#     distance_c = np.random.randint(-10, 10)
#     start_r = math.ceil((r-min_sz)/2) + distance_r
#     start_c = math.ceil((c-min_sz)/2) + distance_c
#     return crop(im, start_r, start_c, min_sz)
...
class NearCenterCrop(CoordTransform):
    """ A class that represents a Near Center Crop.
    ...
    def set_state(self):
        self.store.distance_r = np.random.randint(-10, 11)
        self.store.distance_c = np.random.randint(-10, 11)

    def do_transform(self, x, is_y):

        # return near_center_crop(x, self.sz_y if is_y else self.min_sz)

        r, c, *_ = x.shape
        min_sz = self.sz_y if is_y else self.min_sz
        if min_sz is None: min_sz = min(r, c)
        start_r = math.ceil((r - min_sz) / 2) + self.store.distance_r
        start_c = math.ceil((c - min_sz) / 2) + self.store.distance_c
        return crop(x, start_r, start_c, min_sz)
...
crop_fn_lu = {CropType.RANDOM: RandomCrop, CropType.CENTER: CenterCrop, CropType.NO: NoCrop, CropType.GOOGLENET: GoogleNetResize, CropType.FIVE: FiveCrop, CropType.IDENTITY: Identity, CropType.NEARCENTER: NearCenterCrop}
