...
class auxilary:
    ...
    def heads_eval_mode(m):
        m.eval()
        for modules in [m.bconv0, m.bconv1, m.bconv2, m.bconv3, m.drop, m.sconv0, m.sconv1, m.sconv2, m.sconv3, m.out]:
    ...
    def heads_train_mode(m):
        ...
        if auxilary.auxilary.fine_tune[0] == 0:
            ...
            for branch in auxilary.auxilary.fine_tune[1:auxilary.auxilary.fine_tune[0]]:
                for module in [m.bconv0[branch], m.bconv1[branch], m.bconv2[branch], m.bconv3[branch], m.drop[branch],
                               m.sconv0[branch], m.sconv1[branch], m.sconv2[branch], m.sconv3[branch], m.out[branch]]:
                    module.train()
            if auxilary.auxilary.fine_tune_wgen[0] == 1:
                branch = auxilary.auxilary.fine_tune_wgen[1]
                for module in [m.bconv0[branch], m.bconv1[branch], m.bconv2[branch], m.bconv3[branch], m.drop,
                               m.sconv0[branch], m.sconv1[branch], m.sconv2[branch], m.sconv3[branch], m.out[branch]]:
                    module.train()