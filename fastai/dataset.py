...
def open_image(fn):
    ...
    if not os.path.exists(fn):
        ...
        try:
            ...
            # return np.reshape(im, (492, 508, 1))
...
class ImageData(ModelData):
    def __init__(self, path, datasets, bs, num_workers, classes, len_list_trn=None, len_list_val=None):
        trn_ds,val_ds,fix_ds,aug_ds,test_ds,test_aug_ds = datasets
        self.path,self.bs,self.num_workers,self.classes = path,bs,num_workers,classes
        self.trn_dl,self.val_dl = [
            self.get_dl(ds,shuf,len_list,batch_size) for ds,shuf,len_list,batch_size in [
                (trn_ds,True,len_list_trn,self.bs),(val_ds,False,len_list_val,1)
            ]
        ]
        self.fix_dl,self.aug_dl,self.test_dl,self.test_aug_dl = [
            self.get_dl(ds,shuf,batch_size=batch_size) for ds,shuf,batch_size in [
                (fix_ds,False,self.bs),(aug_ds,False,self.bs),
                (test_ds,False,1),(test_aug_ds,False,1)
            ]
        ]

    def get_dl(self, ds, shuffle, len_list=None, batch_size=1):
        if ds is None: return None
        return DataLoader(ds, batch_size=batch_size, shuffle=shuffle,
            num_workers=self.num_workers, pin_memory=False, len_list=len_list)
...
class ImageClassifierData(ImageData):
    ...
    def from_csv(cls, path, folder, csv_fname, bs=64, tfms=(None,None),
               ...
               len_list_trn=None, len_list_val=None):
        ...
        return cls.from_names_and_array(path, fnames, y, classes, val_idxs, test_name,
                ...
                len_list_trn=len_list_trn, len_list_val=len_list_val)

    @classmethod
    def from_names_and_array(cls, path, fnames,y,classes, val_idxs=None, test_name=None,
            num_workers=8, suffix='', tfms=(None,None), bs=64, continuous=False, len_list_trn=None, len_list_val=None):
        ...
        return cls(path, datasets, bs, num_workers, classes=classes, len_list_trn=len_list_trn, len_list_val=len_list_val)
