...
def scale_min(im, targ, interpolation=cv2.INTER_AREA):
    ...
    if len(im.shape) == 3:
        if im.shape[2] == 1:
            im2 = cv2.resize(im, sz, interpolation=interpolation)
            return np.reshape(im2, (np.shape(im2)[0], np.shape(im2)[1], 1))
...
def rotate_cv_center_coords(yp, im, deg):
    """ Rotates a set of central coordinates by deg degrees around the center of the image
    ...
    y2 = np.copy(yp)
    y2[:,0] = ox + np.cos(angle) * (yp[:, 0] - ox) - np.sin(angle) * (yp[:, 1] - oy)
    y2[:,1] = oy + np.sin(angle) * (yp[:, 0] - ox) + np.cos(angle) * (yp[:, 1] - oy)
    return np.round(y2)
...
def limited_random_central_crop(im, shift_x, shift_y, min_sz=None):
    """ Returns a center crop of an image"""
    r,c,*_ = im.shape
    if min_sz is None: min_sz = min(r,c)
    start_r = math.ceil((r-min_sz)/2) + shift_x
    start_c = math.ceil((c-min_sz)/2) + shift_y
    return crop(im, start_r, start_c, min_sz)


def center_crop_coord_centers(yp, im, sz_y=None):
    """ Returns coords of centeroids of a center crop of an image"""
    yp = np.array(yp, dtype=np.float32)
    r, c, *_ = im.shape
    if sz_y is None: sz_y = min(r, c)

    start_r = math.ceil((r - sz_y) / 2)
    start_c = math.ceil((c - sz_y) / 2)

    yp[:, 0][yp[:, 0] < start_r] = 0
    yp[:, 0][yp[:, 0] >= start_r + sz_y] = 0
    yp[:, 0] = yp[:, 0] - start_r

    yp[:, 1][yp[:, 1] < start_c] = 0
    yp[:, 1][yp[:, 1] >= start_c + sz_y] = 0
    yp[:, 1] = yp[:, 1] - start_c

    zeroList = (yp[:, 0] < 0) | (yp[:, 1] < 0)
    yp[zeroList, 0] = 0
    yp[zeroList, 1] = 0

    # yp = yp.tolist()
    # yp = [np.array(a) for a in yp]

    # # cols,rows = np.nonzero(YY)
    # cols, rows = np.where(YY == 1)
    # if len(cols) == 0:
    ...
    #             if np.abs(cols[i] - cols[j]) < 5 and np.abs(rows[i] - rows[j]) < 5:
    #                 cols = np.delete(cols, i)
    #                 rows = np.delete(rows, i)
    #                 i -= 1
    #                 break
    #         i += 1
    # return [[cols[i], rows[i]] for i in range(0, len(cols))]

    return yp

def limited_random_center_crop_coord_centers(yp, shift_x, shift_y, im, sz_y=None):
    """ Returns coords of centeroids of a center crop of an image"""
    yp = np.array(yp, dtype=np.float32)
    r, c, *_ = im.shape
    if sz_y is None: sz_y = min(r, c)

    start_r = math.ceil((r - sz_y) / 2) + shift_x
    start_c = math.ceil((c - sz_y) / 2) + shift_y
...
def fliplr_coord_centers(yp, im):
...
def five_crop(im, min_sz=None, area=5):
    """ Returns a center crop of an image"""
    if area == 5:
        return center_crop(im, min_sz)
    else:
        r, c, *_ = im.shape
        if min_sz is None: min_sz = min(r, c)
        if area == 1:
            start_r = math.ceil((r - min_sz))
            start_c = math.ceil((c - min_sz))
        elif area == 2:
            start_r = 0
            start_c = math.ceil((c - min_sz))
        elif area == 3:
...
class TfmType(IntEnum):
    ...
    Parameters
        IntEnum: predefined types of transformations
            ...
            COORD_CENTERS: y are coordinates of the centers
    """
    NO = 1
    PIXEL = 2
    COORD = 3
...
def to_centers(YY, y="deprecated"):
    """Convert mask YY to a center coordinate, assumes 0 as background nonzero object"""
    # cols,rows = np.nonzero(YY)
    cols, rows = np.where(YY == 1)
    if len(cols) == 0: return [np.zeros(2, dtype=np.float32)]
    elif len(cols) > 1:
        i = 0
        while i < len(cols) - 1:
            for j in range(i+1, len(cols)):
            ...
            i += 1
    return [[cols[i], rows[i]] for i in range(0, len(cols))]
    # row = (np.min(rows) + np.max(rows)) // 2
    # col = (np.min(cols) + np.max(cols)) // 2
    # return np.array([col, row], dtype=np.float32)
    # return np.array([cols[0], rows[0]], dtype=np.float32)
    # if len(cols) == 0: return np.zeros(2, dtype=np.float32)
...
class Transform():
    ...
    def __call__(self, x, y):
        ...
        x,y = ((self.transform(x),y) if self.tfm_y==TfmType.NO
                ...
                else self.transform_coord_centers(x,y) if self.tfm_y==TfmType.COORD_CENTERS
                else self.transform_coord(x,y) if self.tfm_y==TfmType.COORD
                else self.transform_coord_centers(x,y))
        return x, y
...
class CoordTransform(Transform):
    ...
    def map_y_centers(self, y0, x):
        y = CoordTransform.make_square_centers(y0, x)
        y_tr = self.do_transform(y, True)
        return to_centers(y_tr)
        # return np.array(np.unravel_index(y_tr.argmax(), y_tr.shape), dtype=np.float32)

    def transform_coord_centers(self, x, ys):
        yp = partition(ys, 2)
        if self.name == "CenterCrop":
            # y2 = [self.map_y_centers(y, x) for y in yp]
            y2 = center_crop_coord_centers(yp, x, self.sz_y)
        elif self.name == "LimitedRandomCentralCrop":
            r, c, *_ = x.shape
            if r >= 522:
                self.store.shift_x = int(round(((522 - r) // 2) * self.store.shift_ratio_x))
            else:
                self.store.shift_x = 0
            if c >= 522:
                self.store.shift_y = int(round(((522 - c) // 2) * self.store.shift_ratio_y))
            else:
                self.store.shift_y = 0
            y2 = limited_random_center_crop_coord_centers(yp, self.store.shift_x, self.store.shift_y, x, self.sz_y)
        elif self.name == "RandomRotate":
            # y2 = [self.map_y_centers(y, x) for y in yp]
            if self.store.rp:
                y2 = rotate_cv_center_coords(yp, x, self.store.rdeg)
            else:
                y2 = yp
        elif self.name == "RandomFlip":
            # y2 = [self.map_y_centers(y, x) for y in yp]
            if self.store.do_flip:
                y2 = fliplr_coord_centers(np.copy(yp), x)
            else:
                y2 = np.array(np.copy(yp), dtype=np.int64)
        else:
            y2 = [self.map_y_centers(y,x) for y in yp]
        x = self.do_transform(x, False)
        return x, np.concatenate(y2).reshape(-1)
...
class LimitedRandomCentralCrop(CoordTransform):
    """ A class that represents a Limited Random Central Crop.
    ...
    def __init__(self, sz, tfm_y=TfmType.NO, sz_y=None):
        super().__init__(tfm_y)
        self.min_sz,self.sz_y = sz,sz_y
        self.name = "LimitedRandomCentralCrop"

    def set_state(self):
        self.store.shift_ratio_x = random.uniform(-1, 1)
        self.store.shift_ratio_y = random.uniform(-1, 1)
    ...
    def do_transform(self, x, is_y):
        if not is_y:
            ...
            if c >= 522:
                ...
                self.store.shift_y = 0
        return limited_random_central_crop(x, self.store.shift_x, self.store.shift_y, self.sz_y if is_y else self.min_sz)


class FiveCrop(CoordTransform):
    """ A class that represents a Center Crop.
    ...
    ---------
        ...
        area : int
            Chosen area to crop.
                1: top-right, 2: top-left 3: bottom-left, 4:bottom-right, 5:center
    """
    def __init__(self, sz, tfm_y=TfmType.NO, sz_y=None, area=5):
        super().__init__(tfm_y)
        self.min_sz,self.sz_y = sz,sz_y
        self.area = area

    def do_transform(self, x, is_y):
        return five_crop(x, self.sz_y if is_y else self.min_sz, self.area)
...
class Identity(CoordTransform):
    """  A transformation that keeps input as is. """
    def __init__(self, sz, tfm_y=TfmType.NO, sz_y=None):
        super().__init__(tfm_y)
        self.sz,self.sz_y = sz,sz_y
...
crop_fn_lu = {CropType.RANDOM: RandomCrop, CropType.CENTER: CenterCrop, CropType.NO: NoCrop, CropType.GOOGLENET: GoogleNetResize, CropType.FIVE: FiveCrop, CropType.IDENTITY: Identity, CropType.LTDCENTER: LimitedRandomCentralCrop}

class Transforms():
    def __init__(self, sz, tfms, normalizer, denorm, crop_type=CropType.CENTER,
                 tfm_y=TfmType.NO, sz_y=None, area=5):
        if sz_y is None: sz_y = sz
        self.sz,self.denorm,self.norm,self.sz_y = sz,denorm,normalizer,sz_y
        if crop_type == CropType.FIVE:
            crop_tfm = crop_fn_lu[crop_type](sz, tfm_y, sz_y, area=area)
        else:
...
def image_gen(normalizer, denorm, sz, tfms=None, max_zoom=None, pad=0, crop_type=None,
              tfm_y=None, sz_y=None, pad_mode=cv2.BORDER_REFLECT, scale=None, area=5):
    """
    ...
    if (scale is not None) and (not is_listy(scale)): scale = [scale]
    if pad: scale.append(AddPadding(pad, mode=pad_mode))
    if (scale is not None) and (crop_type!=CropType.GOOGLENET): tfms = scale + tfms
    return Transforms(sz, tfms, normalizer, denorm, crop_type,
                      tfm_y=tfm_y, sz_y=sz_y, area=area)
...
def tfms_from_stats(stats, sz, aug_tfms=None, max_zoom=None, pad=0, crop_type=CropType.RANDOM,
                    tfm_y=None, sz_y=None, pad_mode=cv2.BORDER_REFLECT, norm_y=True, scale=None, area=5):
    ...
    if crop_type == CropType.GOOGLENET or crop_type == CropType.CENTER:
        val_crop = CropType.CENTER
    elif crop_type == CropType.FIVE:
        val_crop = CropType.FIVE
        crop_type = CropType.RANDOM
    else:
        val_crop = crop_type
    val_tfm = image_gen(tfm_norm, tfm_denorm, sz, pad=pad, crop_type=val_crop,
            tfm_y=tfm_y, sz_y=sz_y, scale=scale, area=area)
...
def tfms_from_model(f_model, sz, aug_tfms=None, max_zoom=None, pad=0, crop_type=CropType.RANDOM,
    ...
    # line added to support single channel input
    # stats = np.mean(stats, axis=1)
    stats = [1e-6, 1]
    return tfms_from_stats(stats, sz, aug_tfms, max_zoom=max_zoom, pad=pad, crop_type=crop_type,
                           tfm_y=tfm_y, sz_y=sz_y, pad_mode=pad_mode, norm_y=norm_y, scale=scale, area=area)
