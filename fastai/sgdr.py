...
class LossRecorder(Callback):
    ...
    def on_train_begin(self):
        ...
        self.val_losses, self.trn_losses, self.rec_metrics = [], [], []
        if self.record_mom:
            self.momentums = []
        self.iteration = 0
        self.epoch = 0
    ...
    def on_epoch_end(self, metrics):
        ...
        self.plot_loss(n_skip=0, n_skip_end=0)
    ...
    def save_metrics(self,vals):
        ...
        self.val_losses.append(vals[0]) # Due to using pytorch 0.4
        self.trn_losses.append(self.losses[-1])
        if len(vals) > 2: self.rec_metrics.append(vals[1:])
        elif len(vals) == 2: self.rec_metrics.append(vals[1])

    def plot_loss(self, n_skip=10, n_skip_end=5, save_path=''):
        '''
        ...
        if n_skip_end == 0:
            # plt.plot(range(1+n_skip, self.epoch + 1), self.trn_losses[n_skip:])
            plt.plot(range(1+n_skip, self.epoch + 1), self.val_losses[n_skip:])
        else:
            # plt.plot(range(1+n_skip, self.epoch + 1 - n_skip_end), self.trn_losses[n_skip:-n_skip_end])
            plt.plot(range(1+n_skip, self.epoch + 1 - n_skip_end), self.val_losses[n_skip:-n_skip_end])
        if not in_ipynb():
            # plt.savefig(os.path.join(self.save_path, 'loss_plot.png'))
            # np.save(os.path.join(self.save_path, 'losses.npy'), self.losses)
            # np.save(os.path.join(self.save_path, 'val_losses.npy'), self.val_losses)
            plt.savefig(self.save_path + 'loss_plot.png')
            np.save(self.save_path + 'losses.npy', self.losses)
            np.save(self.save_path + 'val_losses.npy', self.val_losses)
...
class CosAnneal(LR_Updater):
    ...
    def __init__(self, layer_opt, nb, on_cycle_end=None, cycle_mult=1, save_path=''):
        self.nb,self.on_cycle_end,self.cycle_mult = nb,on_cycle_end,cycle_mult
        super().__init__(layer_opt, save_path=save_path)
...
class SaveBestModel(LossRecorder):
    ...
    def save_when_only_loss(self, metrics):
        ...
        if self.best_loss == None or loss < self.best_loss:
            ...
            torch.save(self.model.sched.layer_opt.opt.state_dict(), os.path.join(self.model.models_path, self.name) + '_opt.h5')
...
class OptimScheduler(LossRecorder):
    ...
    def __init__(self, layer_opt, phases, nb_batches, stop_div = False, save_path=''):
        self.phases, self.nb_batches, self.stop_div = phases, nb_batches, stop_div
        super().__init__(layer_opt, record_mom=True, save_path=save_path)
