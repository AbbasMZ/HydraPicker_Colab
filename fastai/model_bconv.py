...
from .swa import *
from .fp16 import *
from fastai import auxilary_bconv
...
class Stepper():
    ...
    def reset(self, train=True):
        if train:
            ...
            try:
                auxilary_bconv.auxilary.heads_train_mode(self.m[1])
            except:
                pass
        else:
            self.m.eval()
            try:
                auxilary_bconv.auxilary.heads_eval_mode(self.m[1])
            except:
                pass
...
def set_train_mode(m):
    if (hasattr(m, 'running_mean') and (getattr(m,'bn_freeze',False)
        ...
        m.eval()
        try:
            auxilary_bconv.auxilary.heads_eval_mode(m[1])
        except:
            pass
        ...
        if auxilary_bconv.auxilary.heads_training_only[0] == 0:
            m.train()
        try:
            auxilary_bconv.auxilary.heads_train_mode(m[1])
        except:
            pass
    ...
    for epoch in tnrange(tot_epochs, desc='Epoch'):
        ...
        for (*x,y) in t:
            ...
            if particle_flag == True:
                ...
                auxilary_bconv.auxilary.Tparticle[0] = int(y[2][0]) * torch.eye(1, dtype=torch.int8)
                # particle[0] = y[2][0]
        ...
        if not all_val:
            if particle_flag == True:
                vals = validate(model_stepper, cur_data.val_dl, metrics, particle_flag)
            else:
...
def validate_next(stepper, metrics, val_iter):
    ...
    with no_grad_context():
        ...
        # res += [f(preds, y) for f in metrics]
    stepper.reset(True)
    return res

def validate(stepper, dl, metrics, particle=None):
    batch_cnts,loss,res = [],[],[]
    stepper.reset(False)
    with no_grad_context():
        for (*x,y) in iter(dl):
            ...
            res.append([f(preds, y) for f in metrics])
