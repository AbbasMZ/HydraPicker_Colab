    ...
    def heads_train_mode(m):
        ...
        if aux.fine_tune[0] == 0:
            ...
            for modules in [m.drop, m.sconv0, m.sconv1, m.sconv2, m.sconv3, m.out]:
                for module in modules:
                    module.eval()
            for branch in aux.fine_tune[1:aux.fine_tune[0]]:
                for module in [m.drop[branch], m.sconv0[branch], m.sconv1[branch], m.sconv2[branch], m.sconv3[branch], m.out[branch]]:
                    module.train()
            if aux.fine_tune_wgen[0] == 1:
                branch = aux.fine_tune_wgen[1]
                for module in [m.drop[branch], m.sconv0[branch], m.sconv1[branch], m.sconv2[branch], m.sconv3[branch], m.out[branch]]:
                    module.train()
            if aux.fine_tune_wfine[0] == 1:
                branch = aux.fine_tune_wfine[1]
                for module in [m.drop[branch], m.sconv0[branch], m.sconv1[branch], m.sconv2[branch], m.sconv3[branch], m.out[branch]]:
                    module.train()